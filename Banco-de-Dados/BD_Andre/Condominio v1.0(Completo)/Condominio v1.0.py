...
def consulta1():
    n = 2
    requisicao = "SELECT CodUH,Condomino_Cod_Condomino FROM unidadeshabitacionais " \
                 "WHERE Condominio_CodCondominio = '"+str(n)+"'"
    ponteiro.execute(requisicao)
    for i in ponteiro:
        print("Código da Unidade = "+str(i[0])+"  Condomino = "+str(i[1]))
def consulta2():
    n = 100
    requisicao = "SELECT condominio.Nome FROM condominio,unidadeshabitacionais " \
                 "WHERE condominio.CodCondominio = unidadeshabitacionais.Condominio_CodCondominio " \
...
def consulta3():
    ...
    requisicao = "SELECT cadastrogeraldepessoas.Nome " \
                 ...
                 "AND unidadeshabitacionais.Condomino_Cod_Condomino = sindico.Condominio_CodCondominio " \
                 "AND unidadeshabitacionais.Area > '"+str(n)+"'"
    ponteiro.execute(requisicao)
    for i in ponteiro:
        print(i)
